...
def iterative_training(all_texts, max_iterations=50, margin=1.0, temperature=0.8, lambda_t=1.0, batch_size=32):
    """
    ...
    while True:
        eps = float(input(
            f"Enter the eps value for initial clustering (default suggestion: {mean_distance * 0.75:.4f}): ") or mean_distance * 0.5)
    ...
    for iteration in range(max_iterations):
        ...
        while True:
            eps = float(input(
                f"Enter the eps value for clustering (default suggestion: {mean_distance * 0.75:.4f}): ") or mean_distance * 0.5)
    ...
    return adjusted_labels, cluster_labels


def main():
    # Load the labeled dataset
    labeled_data_path = "data/labeled_data/labeled_texts_mixed.csv"  # Path to the labeled CSV file
    labeled_data = pd.read_csv(labeled_data_path)

    # Clean and validate TEXT column
    labeled_data['Content'] = labeled_data['Content'].fillna('').astype(str).str.replace(r'[\ufe0f\x0f\u0964]', '',
                                                                                         regex=True)
    labeled_data = labeled_data[labeled_data['Content'].str.strip() != '']

    # Sample and prepare the data
    sampled_labeled_data = labeled_data.sample(n=100, random_state=11)  # Randomly sample # rows
    all_labeled_texts = sampled_labeled_data['Content'].tolist()
    true_labels = sampled_labeled_data['Label'].tolist()
    # Convert true_labels to integers (if they are strings)
    true_labels = [int(label) for label in true_labels]

    # Run iterative training
    adjusted_labels, cluster_labels = iterative_training(all_labeled_texts, max_iterations=50, batch_size=32)

    # Map cluster labels to text instances (including handling noise points)
    # Ensure cluster_labels uses integers
    cluster_labels = {int(k): int(v) if v != 'Noise' else 0 for k, v in cluster_labels.items()}

    # Map cluster labels to text instances (including handling noise points)
    predicted_labels_numerical = [int(cluster_labels[label]) if label != -1 else 0 for label in adjusted_labels]

    # Compare predicted labels with true labels
    from sklearn.metrics import precision_score, recall_score

    precision = precision_score(true_labels, predicted_labels_numerical)
    recall = recall_score(true_labels, predicted_labels_numerical)
